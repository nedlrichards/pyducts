...
from scipy.linalg import solve, solve_banded, sqrtm, expm
from math import pi
...
oper = expm(1j * k0 * deltar * (sqrtm(Q) - np.identity(nz)))
# ******************************************************
# March out in range
# ******************************************************
for ir in range(nr - 1):
   # equivalent to psi( :, ir+1 ) = exp(1j * k0 * dr * (Q -1)) * psi( :, ir );
    psi[:, ir+1] = oper @ psi[:, ir]
...
tl = 20 * np.log10(np.abs(psi * hank))
